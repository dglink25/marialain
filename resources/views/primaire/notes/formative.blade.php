...
<div class="min-h-screen bg-gray-50 py-4 px-3 sm:py-6 sm:px-6 lg:px-8">
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div class="flex items-center gap-3">
            <div class="w-10 h-10 bg-emerald-500 rounded-xl flex items-center justify-center shadow-md shrink-0">
                <i class="fas fa-pencil-alt text-white"></i>
            </div>
            <div>
                <h1 class="text-xl sm:text-2xl font-bold text-gray-900">Évaluations formatives</h1>
                <p class="text-xs sm:text-sm text-gray-500 mt-0.5">
                    Classe : <span class="font-semibold text-emerald-700">{{ $classe->name }}</span> — {{ $annee_academique->name }}
                </p>
            </div>
        </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-10 text-center">
        <i class="fas fa-clipboard text-emerald-300 text-4xl mb-3"></i>
        <p class="text-gray-500 font-medium">Aucune évaluation formative enregistrée pour cette classe.</p>
    </div>
    @else
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-5 py-3 text-center">
                            <a href="{{ route('primaire.notes.formative.show', [$classe->id, $eval->id]) }}"
