...
<div class="min-h-screen bg-gray-50 py-4 px-3 sm:py-6 sm:px-6 lg:px-8">
        ...
        <div class="flex items-center gap-3 min-w-0">
            ...
            <div class="min-w-0">
                <h1 class="text-xl sm:text-2xl font-bold text-gray-900 truncate">
                    Récapitulatif — Évaluations formatives
                </h1>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div class="px-4 sm:px-6 py-4 border-b border-gray-100 flex items-center gap-2">
            ...
            <h2 class="text-base font-bold text-gray-800">Moyenne des évaluations formatives, toutes matières</h2>
        </div>
            ...
            <table class="w-full text-sm min-w-[720px]">
                    ...
                    <tr>
                        ...
                        <th class="px-4 py-2.5 text-center whitespace-nowrap">
                            ...
                            <span class="block text-[10px] font-normal normal-case text-gray-400">
                                {{ $nbEvalParMatiere[$subject->id] }} éval.
                            </span>
                        </th>
                        @endforeach
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    @php
                        $moyennesMatieres = [];
                        foreach ($subjects as $subject) {
                            $m = $matrice[$student->id][$subject->id] ?? null;
                            if ($m !== null) { $moyennesMatieres[] = $m; }
                        }
                        $moyenneEleve = count($moyennesMatieres) > 0
                            ? array_sum($moyennesMatieres) / count($moyennesMatieres)
                            : null;
                    @endphp
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        @foreach($subjects as $subject)
                        @php $moy = $matrice[$student->id][$subject->id] ?? null; @endphp
                        <td class="px-4 py-2.5 text-center">
                            @if($moy !== null)
                                <span class="font-semibold text-gray-700">{{ number_format($moy, 1, ',', '') }}</span>
                            @else
        ...
        <div class="px-4 sm:px-6 py-3 border-t border-gray-100 bg-gray-50 text-xs text-gray-500">
            ...
            Chaque case affiche la moyenne de toutes les évaluations formatives déjà saisies pour la matière,
            ramenée sur /20 pour permettre la comparaison entre matières ayant des barèmes différents (5/10 ou 10/20).
        </div>
    </div>
    @endif
