...
<div class="container mx-auto px-4 py-8">
    ...
    <div id="cahier-modal" class="hidden fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 transition-opacity duration-300">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl mx-auto p-6 relative animate-fadeInUp max-h-[90vh] overflow-y-auto">
            ...
            <form id="modal-form" method="POST" class="space-y-4">
                ...
                <div class="flex items-center space-x-3 p-4 bg-gray-50 rounded-lg border border-gray-200">
                    <input type="checkbox" id="is_late_checkbox" name="is_late" value="1" 
                        class="w-4 h-4 text-red-600 bg-gray-100 border-gray-300 rounded focus:ring-red-500 focus:ring-2"
                        onchange="toggleLateReason()">
                    <label for="is_late_checkbox" class="text-sm font-semibold text-gray-700 cursor-pointer">
                        Marquer comme étant en retard
                    </label>
                </div>

                {{-- Late Reason Box --}}
                <div id="late_reason_box" class="hidden p-4 bg-red-50 border border-red-200 rounded-lg transition-all duration-300 transform origin-top">
                    ...
                    <textarea name="motif_retard" id="motif_retard" rows="3" 
                        class="w-full border-red-300 rounded-lg p-3 text-sm focus:ring-2 focus:ring-red-500 focus:border-red-500 transition-all"
                        placeholder="Veuillez expliquer le motif du retard..."
                        required></textarea>
                    <p class="text-xs text-red-600 mt-2">Ce champ devient obligatoire lorsque le cours est marqué comme étant en retard.</p>
                </div>
                        ...
                        <tr class="hover:bg-indigo-50 transition-colors duration-200">
                            ...
                            <td class="px-6 py-4">
                                @if($entry->is_late)
                                    ...
                                    <div class="text-xs text-gray-600 mt-1 max-w-xs">{{ Str::limit($entry->motif_retard, 50) }}</div>
                                @else
                            ...
                            <td class="px-6 py-4">
                                ...
                                @else
                                    <span class="inline-flex items-center px-3 py-1.5 border border-gray-300 text-gray-400 bg-gray-50 rounded-lg text-xs">
                                        <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <div class="text-xs text-gray-500 mt-2">
                                    Modifié : {{ $entry->updated_at->diffForHumans() }}
                                </div>
                            </td>
                        </tr>
                        @endforeach
