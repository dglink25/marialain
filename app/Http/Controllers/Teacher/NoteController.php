...
class NoteController extends Controller{
    public function index($classId, $subjectId, $trimestre)
    {
        ...
        $permission = \App\Models\NotePermission::where('class_id', $classId)
            ->where('trimestre', (string) $trimestre)
            ->first();
    ...
    public function store(Request $request, $classId, $subjectId, $type, $num, $trimestre) {
        ...
        // ── Vérification que l'enseignant enseigne bien cette matière ──
        $check = DB::table('class_teacher_subject')
            ->where('class_id', $classId)
            ->where('teacher_id', Auth::id())
            ->where('subject_id', $subjectId)
        ...
        // ── Compteurs pour le message flash ──
        $saved      = 0;
        $notifSent  = 0;
        $notifFailed = 0;
        ...
        foreach ($classe->students as $student) {
            ...
            $noteValue = $request->notes[$student->id];
            ...
            // ── Notification WhatsApp au parent ──
            // On cherche le parent par numéro de téléphone stocké sur l'élève
            if (! is_null($noteValue) && $noteValue !== '') {
                $this->notifyParent(
                    student:      $student,
                    subject:      $subject,
                    classe:       $classe,
        ...
        // ── Message flash avec résumé des notifications ──
        $successMsg = "Notes enregistrées avec succès ({$saved} élèves).";
        if ($notifSent > 0) {
            $successMsg .= " {$notifSent} notification(s) WhatsApp envoyée(s).";
        }
        if ($notifFailed > 0) {
            $successMsg .= " {$notifFailed} notification(s) non envoyée(s) (voir logs).";
        }
    ...
    void {
        // ── Trouver le numéro du parent ──
        // Stratégie 1 : via la relation ParentUser (parent_phone sur l'élève)
        $parentPhone = $student->parent_phone ?? null;

        if (! $parentPhone) {
            Log::info('[Note Notif] Pas de numéro parent', [
                'student_id' => $student->id,
        ...
        $studentName = $student->full_name;        // accesseur défini dans Student.php
        $className   = $classe->name;
        $subjectName = $subject->name;
        $yearLabel   = $academicYear->name;        // ex: "2024-2025"

        $sent = WhatsAppService::sendNoteNotification(
        ...
        if ($sent) {
            ...
            $notifFailed++;
            Log::warning('[Note Notif] Échec envoi WhatsApp', [
                'student'      => $studentName,
    ...
    public function edit($classId, $subjectId, $type, $num, $trimestre) {
        ...
        if (!$permission) {
            return back()->with('error', "Vous n'êtes pas autorisé à modifier les notes de $type $num pour ce trimestre. 
    ...
    public function update(Request $request, $classId, $subjectId, $num, $type, $trimestre){
        ...
        foreach ($request->notes as $studentId => $val) {
            ...
            if (is_null($val) || $val === '') {
                ...
                Grade::updateOrCreate($criteria, ['value' => $val]);
            }
        }
    ...
    public function destroy($classId, $type, $num, $trimestre) {
        ...
        return redirect()
            ->route('teacher.classes.notes.read', [$classId, $type, $num, $trimestre])
    ...
    public function showClassNotes($classId, $subjectId, $trimestre)
    {
        ...
        foreach ($classe->students as $student) {
            ...
            foreach ($grades as $grade) {
                if ($grade->type === 'interrogation') $interros[$grade->sequence] = $grade->value;
                if ($grade->type === 'devoir')        $devoirs[$grade->sequence]  = $grade->value;
            }
